...
/** Usage: java LicenseKeyGenerator <machineId> <path-to-private_key.der> <TODAY|MONTHLY|YEARLY> */
public class LicenseKeyGenerator {
    public static void main(String[] args) throws Exception {
        if (args.length != 3) {
            System.err.println("Usage: <machineId> <path-to-private_key.der> <TODAY|MONTHLY|YEARLY>");
            System.exit(2);
        }
        String machineId = args[0];
        Plan plan = Plan.valueOf(args[2].trim().toUpperCase());
        LocalDate issuedAt = LocalDate.now();
        LocalDate expiresAt = switch (plan) {
            case TODAY -> issuedAt;
            case MONTHLY -> issuedAt.plusMonths(1);
            case YEARLY -> issuedAt.plusYears(1);
        };
    ...
    private enum Plan { TODAY, MONTHLY, YEARLY }
}